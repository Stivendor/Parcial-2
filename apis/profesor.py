...
def listar_profesores(db: Session = Depends(get_db)):
    """
    Lista todos los profesores registrados (incluyendo datos de persona).
    """
...
def obtener_profesor(profesor_id: str, db: Session = Depends(get_db)):
    ...
    """
    profesor = (
        db.query(crud_profesor.Profesor)
        .filter(crud_profesor.Profesor.id_profesor == profesor_id)
        .first()
    )
    if not profesor:
...
def eliminar_profesor(profesor_id: str, db: Session = Depends(get_db)):
    """
    Elimina un profesor y su persona asociada de la base de datos.
    """
